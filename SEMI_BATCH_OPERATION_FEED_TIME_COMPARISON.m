...
%% -1. Loop through different feed times:
disp('-1. Loop through different feed times')

% In this script the simulation times are double the feed times:
FEED_TIMES = [0.5 1 2 5 10 20]*60*60; %[s]

% Colormap:
cmap = parula(length(FEED_TIMES));

for k = 1:length(FEED_TIMES)
  
    disp(sprintf('FEED TIME = %.2e [s]',FEED_TIMES(k)))
    ...
    % Simulation time:
    SIMULATION_TIME = 4*FEED_TIMES(k);     %[s]
    ...
    SAVENAME = 'SEMI-BATCH_OPERATION_FEED_TIME_COMPARISON';
    ...
    % Conversion Plot:
    figure(1)
    plot(ts,Conversion,'Linewidth',1,'Color',cmap(k,:)); grid on; hold on
    xlabel('time [s]')
    ylabel('\chi(Monomer conversion) [-]')
    title('Monomer conversion vs. time:')
    leg1{k} = sprintf('t_{feed} = %.2e',FEED_TIMES(k)); 
    
    % Polymer composition plot:
    figure(2)
    plot(Conversion,[Y_As_inst],'Linewidth',1,'LineStyle','--','Color',cmap(k,:)); grid on; hold on
    plot(Conversion,[Y_As],'Linewidth',1,'LineStyle','-','Color',cmap(k,:)); grid on
    xlabel('\chi [-]')
    ylabel('Y_A [-]')
    title('Polymer composition vs. conversion:')
    leg2{2*(k-1)+1} = sprintf('t_{feed} = %.2e - instantaneous',FEED_TIMES(k));
    leg2{2*(k-1)+2} = sprintf('t_{feed} = %.2e - cumulated',FEED_TIMES(k));
 
end

% Legends:
figure(1); legend(leg1);
figure(2); legend(leg2);
