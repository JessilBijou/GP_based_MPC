...
%CONTROLLED_SEMI_BATCH_OPERATION.m: semi-batch model in power-feed 
%   operation.
...
SAVENAME = 'CONTROLLED_SEMI-BATCH_OPERATION';
...
K_C = 0.1;      %[-] Proportional constant
K_I = 0.05;     %[-] Integral constant
...
% Measured variable setpoint for the controller:
v_SP = fzero(@(X_A_span)Mayo_Lewis_equation(r,X_A_span)-Y_A,Y_A); %[-]

% Manipulated variable bias for the controller:
u_bias = Fin0(2)/(Fin0(2)+Fin0(3)); %[-]

%% 3. ODE FUNCTION AND INITIAL CONDITIONS:
disp('3. ODE FUNCTION AND INITIAL CONDITIONS')

% Initial conditions:
Y0 = [I0 A0 B0 0 0 0 0 V0 0 0 S0 0];

% ODE:
ODE = @(t,Y)controlled_feed_semi_batch_ODE(t,Y,T_K,A_B_SYSTEM,Fin0,FEED_TIME,v_SP,K_C,K_I,u_bias,SATURATION);
...
% Integration:
[ts,Ys,te,Ye] = ode15s(ODE,t_span,Y0,options);
...
% Feed rates and control parameters:
for i=1:length(ts)
   [~,v_SP(i,:),v(i,:),e(i,:),Fin_struct] = ODE(ts(i),Ys(i,:));
   Fin(i,:) = [Fin_struct.I Fin_struct.A Fin_struct.B Fin_struct.S]; %[mol/s]
end
...
figure 
subplot(2,1,1)
p=plot(ts,[v_SP v],'Linewidth',1);
grid on;
p(1).LineStyle = '--';
xlabel('ts [s]')
ylabel('X_A [-]')
title('Measured variable 1: X_A')
legend('Setpoint', 'Measured')

subplot(2,1,2)
plot(ts,Fin,'Linewidth',1);
grid on;
xlabel('t [s]')
ylabel('Fin_i [mol/s]')
title('Controlled variable: Fin')
legend('I','A','B','S')
